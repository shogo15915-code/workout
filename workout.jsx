...
async function loadAll() {
  const result = { logs: {}, exercises: null, bodyWeightKg: DEFAULT_BODY_WEIGHT, goalsMemo: "", exerciseNotes: {} };
  try {
  ...
  try {
    const nRes = await window.storage.get("exercise-notes", false);
    if (nRes && nRes.value) result.exerciseNotes = JSON.parse(nRes.value);
  } catch (e) {}
  return result;
}
...
async function saveExerciseNotes(notes) {
  try {
    await window.storage.set("exercise-notes", JSON.stringify(notes), false);
  } catch (e) {}
}
...
function SetRow({ index, set, onChange, onRemove, prevSet }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0" }}>
      ...
      {(() => {
        const rm = estOneRM(Number(set.weight), Number(set.reps), isBW);
        return rm > 0 ? (
          <div style={{ width: 44, flexShrink: 0, textAlign: "right", fontSize: 11, color: COLORS.accent, fontFamily: "Inter, sans-serif" }}>
            {rm.toFixed(1)}
          </div>
        ) : (
...
function ExerciseCard({ entry, prevEntry, bestEverRM, note, onNoteChange, onUpdate, onDelete }) {
    ...
    <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 12, padding: "14px 14px 10px", marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {open && (
            <input
              ...
              placeholder="コツ・メモ"
              style={{ ...noteInputStyle, width: "100%", marginTop: 6 }}
            />
          )}
          {open && prevEntry && (
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          <button onClick={() => setOpen(!open)} style={iconBtnStyle} aria-label={open ? "折りたたむ" : "展開する"}>
            <span style={{ transform: open ? "rotate(0deg)" : "rotate(180deg)", transition: "transform 0.15s", display: "flex" }}>
              <IconChevron size={16} />
            </span>
          </button>
          {bestEverRM > 0 && (
            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: "Bebas Neue, Inter, sans-serif", fontSize: 18, color: COLORS.accent, lineHeight: 1 }}>
                {bestEverRM.toFixed(1)}
              </div>
              <div style={{ fontSize: 9, color: COLORS.textMute, letterSpacing: 0.5 }}>自己PR</div>
            </div>
          )}
...
/* ---------- Cardio logging card ---------- */
function CardioCard({ entry, prevEntry, note, onNoteChange, onUpdate, onDelete, bodyWeightKg }) {
  const sets = entry.sets;
  const [open, setOpen] = useState(true);
  const updateSet = (i, newSet) => {
    const newSets = sets.slice();
        ...
        return (
          ...
          >
            <div>
              ...
              <div style={{ color: COLORS.textMute, fontSize: 12, marginTop: 3 }}>{parts.join(" ・ ")}</div>
            </div>
  ...
  return (
    <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 14, padding: "14px 12px", marginBottom: 12 }}>
      <div
        ...
        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: open ? 10 : 0, padding: "0 4px", cursor: "pointer" }}
      >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <span style={{ transform: open ? "rotate(-90deg)" : "rotate(90deg)", transition: "transform 0.15s", display: "flex" }}>
            <IconChevron size={16} />
          </span>
        </div>
      </div>
...
export default function KintoreMemo() {
  ...
  const findBestEverRM = (exerciseName) => {
    let best = 0;
    Object.keys(logs).forEach((d) => {
      (logs[d] || []).forEach((e) => {
        if (e.exerciseName !== exerciseName || e.bodyPart === "cardio") return;
        e.sets.forEach((s) => {
          best = Math.max(best, estOneRM(Number(s.weight), Number(s.reps), s.bw));
        });
      });
    });
    return best;
  ...
  const handleSaveExerciseNote = (exerciseName, text) => {
    const next = { ...exerciseNotes, [exerciseName]: text };
    setExerciseNotes(next);
    saveExerciseNotes(next);
  };
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh", color: COLORS.text, fontFamily: "Inter, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      {/* Body */}
      <div style={{ flex: 1, padding: "16px 16px 170px", overflowY: "auto" }}>
        {tab === "log" && (
          <>
              ...
              entriesForDate.map((entry, idx) =>
                entry.bodyPart === "cardio" ? (
                  <CardioCard
                    key={idx}
                    entry={entry}
                    prevEntry={findPrevEntry(entry.exerciseName)}
                    bodyWeightKg={bodyWeightKg}
                    note={exerciseNotes[entry.exerciseName]}
                    onNoteChange={(text) => handleSaveExerciseNote(entry.exerciseName, text)}
                    onUpdate={(u) => handleUpdateEntry(idx, u)}
                  ...
                  <ExerciseCard
                    ...
                    entry={entry}
                    prevEntry={findPrevEntry(entry.exerciseName)}
                    bestEverRM={findBestEverRM(entry.exerciseName)}
                    note={exerciseNotes[entry.exerciseName]}
                    onNoteChange={(text) => handleSaveExerciseNote(entry.exerciseName, text)}
                    onUpdate={(u) => handleUpdateEntry(idx, u)}
                    onDelete={() => handleDeleteEntry(idx)}
      ...
      {/* Fixed footer: add-exercise button (log tab only) + bottom nav */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, maxWidth: 640, margin: "0 auto" }}>
        {tab === "log" && (
          <div style={{ padding: "10px 16px 0", background: COLORS.bg }}>
            <button
              onClick={() => setPickerOpen(true)}
              style={{ width: "100%", background: COLORS.accent, border: "none", borderRadius: 12, padding: "14px", color: "#161310", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 15, cursor: "pointer" }}
            >
              ＋ 種目を追加
            </button>
          </div>
        )}
        <div style={{ background: COLORS.surface, borderTop: `1px solid ${COLORS.border}`, display: "flex", padding: "10px 8px calc(10px + env(safe-area-inset-bottom, 0px))" }}>
          {[
            { id: "log", label: "記録" },
            { id: "history", label: "履歴" },
          ].map((t) => (
