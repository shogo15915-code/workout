...
function ExerciseCard({ entry, prevEntry, bestEverRM, onUpdate, onDelete }) {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
            ...
            {bestEverRM > 0 && (
              <div style={{ fontSize: 12, color: COLORS.accent, fontFamily: "Inter, sans-serif" }}>
                自己ベスト {bestEverRM.toFixed(1)}kg
              </div>
            )}
          </div>
          <input
            ...
            {entriesForDate.length === 0 ? (
              ...
              entriesForDate.map((entry, idx) =>
                entry.bodyPart === "cardio" ? (
                  ...
                  <ExerciseCard key={idx} entry={entry} prevEntry={findPrevEntry(entry.exerciseName)} bestEverRM={findBestEverRM(entry.exerciseName)} onUpdate={(u) => handleUpdateEntry(idx, u)} onDelete={() => handleDeleteEntry(idx)} />
                )
              )
            )}
          </>
